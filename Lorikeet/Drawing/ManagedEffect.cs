...
namespace Lorikeet.Effects {
    ...
    public class ManagedEffect {
        ...
        public virtual void draw_texture(Texture2D texture, Vector2 position, Vector2 size) {
            begin_spritebatch();
            Drawing.sb.Draw(texture, new Rectangle(position.ToPoint(), size.ToPoint()), Color.White);
            Drawing.end();
        }
        public virtual void draw_texture(Texture2D texture, Vector2 position, Vector2 size, Vector2 crop_position, Vector2 crop_size) {
            begin_spritebatch();
            Drawing.sb.Draw(texture, 
                new Rectangle(position.ToPoint(), size.ToPoint()),
                new Rectangle((int)crop_position.X, (int)crop_position.Y, (int)crop_size.X, (int)crop_size.Y),
                Color.White);
            Drawing.end();
        }
